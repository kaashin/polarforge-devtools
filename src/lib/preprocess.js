      ...
      if (processFile(filename)) {
        ...
        walk(ast, {
          enter(node, parent, prop, index) {
            ...
            if (node.type === "VariableDeclarator" && parent.type === 'VariableDeclaration' && node.init) {
              ...
              if (parent.leadingComments?.length) {
                ...
                if (find) {
                  ...
                  addImport(addImportMap, '$lib/StoreTools.svelte', ["registerStore"])
            ...
            if (node.type === 'Attribute' && node.name === "kuik") {

              // console.log("kuik value: ", node, node.value)
              markup.overwrite(node.start, node.end, "data-testid='test'")
            }
